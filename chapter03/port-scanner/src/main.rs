...
fn main() {
    ...
    if args.len() != 3 {
        error!("Bad number of args. [ipaddr] [scantype]");
        std::process::exit(1);
    }
...
fn build_packet(packet_info: &PacketInfo) -> [u8; TCP_SIZE] {
    // TCPヘッダの作成
    let mut tcp_bnuffer = [0u8; TCP_SIZE];
    let mut tcp_header = MutableTcpPacket::new(&mut tcp_bnuffer[..]).unwrap();
    ...
    let checksum = tcp::ipv4_checksum(
        &tcp_header.to_immutable(),
        &packet_info.my_ipaddr.into(),
        &packet_info.target_ipaddr.into(),
    );
    tcp_header.set_checksum(checksum);
